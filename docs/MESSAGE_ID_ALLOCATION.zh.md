...
分配器首先尝试目标秒的 `slot 0..15`。如果本秒 slot 已满，则搜索相邻秒：
...
2. 在当前唯一域中未被占用；
3. 不越过同一 chat 中相邻 `msgSeq` 消息的 ID 边界。

第三条保证私聊共用账号映射表时，即使其它私聊占用了目标秒附近的 slot，也不能把当前消息插到本 chat 的上一条消息之前或下一条消息之后。
...
历史同步不再从某个中点简单递减，而是直接根据消息时间戳定位 preferred bucket。因此历史页以任意顺序写入时，只要时间戳仍在 epoch 窗口内，最终 ID 的时间顺序保持一致。
