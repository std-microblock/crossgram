...
当前版本的投影一旦创建，Telegram message ID 就保持不变。QQ 发送状态回调可能先给出临时 `msgSeq`，再由最终状态或历史记录补全正确值；这种顺序元数据修正只原地更新 `nativeSequence`，不能重新分配已经对客户端可见的 ID。否则 durable update 中的旧 ID 与撤回时数据库里的新 ID 不一致，会让客户端重放出无法被删除的重复消息。
