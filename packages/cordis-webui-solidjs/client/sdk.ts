...
export function useConnection(): Connection {
  ...
  if (!connection) throw new Error('WebUI components must be mounted inside ConnectionContext')
  return connection
}
...
export function useRpc<T extends object>(id: string): { readonly data: T; readonly ready: boolean } {
  const connection = useConnection()
  const state = useChannel<T>(id)
  const methods = new Map<string, (...args: unknown[]) => Promise<unknown>>()
  const data = new Proxy({} as T, {
    get(_target, key) {
      if (typeof key === 'string' && connection.state.entries[id]?.methods.includes(key)) {
        if (!methods.has(key)) methods.set(key, (...args) => connection.rpc(id, key, args))
  ...
  return { data, get ready() { return state.ready } }
}
