...
export function SchemaField(props: FieldProps) {
  ...
  const value = () =>
    props.value === undefined ? schema().meta.default : props.value
  const disabled = () => props.disabled || schema().meta.disabled
  const issue = () =>
    props.issues.find(
      (issue) => JSON.stringify(issue.path) === JSON.stringify(props.path),
    )?.message
  ...
  createEffect(() => {
    ...
    const current = value()
    const index =
      schema().list?.findIndex(
        (child) =>
          child.type === 'object' &&
          Object.entries(child.dict ?? {}).filter(
            ([, field]) => field.type === 'const',
          ).length > 0 &&
          Object.entries(child.dict ?? {})
            .filter(([, field]) => field.type === 'const')
            .every(([key, field]) => Object.is(current?.[key], field.value)),
      ) ?? -1
  ...
  const childProps = (child: SchemaNode, key: string | number): FieldProps => ({
    ...
    label: typeof key === 'number' ? 'Item ' + (key + 1) : labelFor(key),
    path: [...props.path, key],
    get issues() {
      return props.issues
    },
    ...
    onChange: (next) => props.onChange(setAt(value(), [key], next)),
  ...
  const setJson = (text: string) => {
    try {
      ...
      setJsonError('')
      props.onInvalid(props.path, '')
      props.onChange(next)
    } catch {
      setJsonError('Enter valid JSON')
      props.onInvalid(props.path, 'Enter valid JSON')
    }
  }
  const addKey = () => {
    const name = key().trim()
    if (!name || !safeKey(name) || Object.hasOwn(value() ?? {}, name)) {
      setKeyError('Choose a unique, non-empty key')
      return
    }
    const issues = schema().sKey ? validateSchema(schema().sKey!, name) : []
    if (issues.length) {
      setKeyError(issues[0].message)
      return
    }
    props.onChange({
      ...(value() ?? {}),
      [name]: initialValue(schema().inner!),
    })
    setKey('')
    setKeyError('')
  }
  const render = () => {
    ...
    if ((props.depth ?? 0) > 20)
      return (
        <p class="muted">Nested schema: use the JSON editor for this value.</p>
      )
    ...
    if (node.type === 'object')
      return (
        <div class="schema-object">
          <For
            each={Object.entries(node.dict ?? {}).filter(
              ([, field]) => !field.meta.hidden,
            )}
          >
            {([key, child]) => <SchemaField {...childProps(child, key)} />}
          </For>
        </div>
      )
          ...
          <For each={node.list}>
            {(child) => (
              <SchemaField
                {...props}
                schema={child}
                label={description(child) || props.label}
                depth={(props.depth ?? 0) + 1}
              />
            )}
          </For>
    ...
    if (node.type === 'transform' && node.inner)
      return (
        <SchemaField
          {...props}
          schema={{ ...node.inner, meta: { ...node.inner.meta, ...node.meta } }}
          depth={(props.depth ?? 0) + 1}
        />
      )
    if (node.type === 'union') {
      ...
      return (
        <div class="stack schema-union">
          <select
            id={id}
            aria-label={props.label}
            value={String(constants ? unionIndex(node, value()) : branch())}
            disabled={disabled()}
            onChange={(event) => {
              const index = +event.currentTarget.value
              setBranch(index)
              props.onChange(switchUnionValue(node, value(), index))
            }}
          >
            <For each={node.list}>
              {(child, index) => (
                <option value={String(index())}>
                  {description(child) ||
                    (child.type === 'const'
                      ? String(child.value ?? 'None')
                      : 'Option ' + (index() + 1) + ' · ' + child.type)}
                </option>
              )}
            </For>
          </select>
    ...
    if (node.type === 'array' || node.type === 'tuple') {
      const items = () =>
        node.type === 'tuple'
          ? (node.list ?? [])
          : Array.from(
              { length: Math.min(value()?.length ?? 0, limit()) },
              () => node.inner!,
            )
      return (
          ...
          <Index each={items()}>
            {(child, index) => (
              <div class="schema-item">
                <SchemaField {...childProps(child(), index)} />
                <Show when={node.type === 'array'}>
                  <button
                    type="button"
                    class="icon-button"
                    aria-label={
                      'Remove ' + props.label + ' item ' + (index + 1)
                    }
                    disabled={
                      disabled() ||
                      (value()?.length ?? 0) <= (node.meta.min ?? 0)
                    }
                    onClick={() =>
                      props.onChange(
                        value().filter(
          ...
          <Show when={node.type === 'array'}>
            <div class="toolbar">
              <button
                ...
                disabled={
                  disabled() ||
                  (value()?.length ?? 0) >= (node.meta.max ?? Infinity)
                }
                onClick={() =>
                  props.onChange([
                    ...(value() ?? []),
                    initialValue(node.inner!),
                  ])
                }
              >
                Add item
              </button>
              <Show when={(value()?.length ?? 0) > limit()}>
                <button
                  type="button"
                  class="button outlined"
                  onClick={() => setLimit(limit() + 20)}
                >
                  Show more items ({value().length - limit()} remaining)
                </button>
              </Show>
            </div>
          </Show>
    ...
    if (node.type === 'dict')
      return (
        <div class="schema-list">
          <For each={Object.keys(value() ?? {}).slice(0, limit())}>
            {(key) => (
              <div class="schema-item">
                <SchemaField {...childProps(node.inner!, key)} />
                <button
                  type="button"
                  class="icon-button"
                  disabled={disabled()}
                  ...
                  onClick={() =>
                    props.onChange(setAt(value(), [key], undefined))
                  }
                >
                  ×
                </button>
          ...
          <div class="toolbar">
            <input
              aria-label={props.label + ' new key'}
              placeholder="New key"
              value={key()}
              disabled={disabled()}
            ...
            <Show when={Object.keys(value() ?? {}).length > limit()}>
              <button
                type="button"
                class="button outlined"
                onClick={() => setLimit(limit() + 20)}
    ...
    if (node.type === 'const')
      return (
        <output id={id} class="schema-constant">
    ...
    if (node.type === 'bitset')
      return (
        <div class="schema-flags">
          <For each={Object.entries(node.bits ?? {})}>
            {([key, bit]) => (
              <label>
                <input
                  type="checkbox"
      ...
      return (
        <div class="schema-input-row">
          <input
            id={id}
            type={
              node.meta.role === 'secret' && !revealed()
                ? 'password'
                : node.meta.role === 'link'
                  ? 'url'
                  : node.meta.role === 'datetime'
                    ? 'datetime-local'
                    : 'text'
            }
            value={value() ?? ''}
            autocomplete={node.meta.role === 'secret' ? 'new-password' : 'off'}
            disabled={disabled()}
            aria-invalid={!!issue()}
            aria-describedby={id + '-hint'}
            onInput={(event) => props.onChange(event.currentTarget.value)}
          />
          <Show when={node.meta.role === 'secret'}>
            <button
              class="button outlined"
              type="button"
    ...
    return (
      <div>
        ...
        />
        <small>JSON value. Plugin-specific validation runs on save.</small>
        <Show when={jsonError()}>
          <small class="field-error" role="alert">
            {jsonError()}
          </small>
    ...
    <Show when={!schema().meta.hidden}>
      ...
      >
        <div class="schema-label">
          <label for={id}>
            {props.label}
            <Show when={schema().meta.required}>
...
export function SchemaForm(props: SchemaFormProps) {
  ...
  const [draft, setDraft] = createSignal(
    clone(props.value) ?? initialValue(schema()),
  )
  ...
  const reload = () => {
    setDraft(clone(props.value) ?? initialValue(schema()))
    setBaseline(JSON.stringify(props.value))
    setInputErrors({})
    setError('')
  ...
  createEffect(() => {
    const current = JSON.stringify(props.value)
    untrack(() => {
      if (current !== baseline() && !dirty()) reload()
    })
  })
  guardNavigation(
    () =>
      !dirty() || window.confirm('Discard your unsaved configuration changes?'),
  )
  ...
  const invalid = (path: (string | number)[], error: string) =>
    setInputErrors((value) => ({ ...value, [JSON.stringify(path)]: error }))
  return (
    <form
      class="schema-form stack"
      onSubmit={save}
      novalidate
      aria-label={props.label ?? 'Plugin configuration'}
    >
      <Show when={conflict()}>
        <div class="notice error" role="alert">
          Configuration changed on the server. Reload before saving to avoid
          overwriting another change.{' '}
          <button class="button outlined" type="button" onClick={reload}>
      ...
      <div class="toolbar">
        ...
        >
          {raw() ? 'Use form editor' : 'Edit JSON'}
        </button>
        <span class="muted">{dirty() ? 'Unsaved changes' : 'Up to date'}</span>
      </div>
      <Show
        when={raw()}
        fallback={
          <Show when={{ schema: schema(), revision: revision() }} keyed>
            {({ schema }) => (
              <SchemaField
                schema={schema}
                value={draft()}
                ...
                onChange={(value) => {
                  setDraft(() =>
                    value === undefined ? initialValue(schema) : value,
                  )
                  setSaved(false)
                }}
                onInvalid={invalid}
        ...
        <label class="field">
          ...
          <textarea
            ...
            onInput={(event) => {
              ...
              try {
                setDraft(JSON.parse(event.currentTarget.value))
                invalid(['$json'], '')
              } catch {
      ...
      <Show when={Object.values(inputErrors()).some(Boolean)}>
        <div class="notice error" role="alert">
          Correct invalid JSON before saving.
        </div>
      </Show>
      <Show when={error()}>
        <div class="notice error" role="alert">
