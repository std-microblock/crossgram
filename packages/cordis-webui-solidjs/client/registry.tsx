...
const extensions = new Map<string, { component: Component<PageProps>; entryId: string; links: HTMLLinkElement[] }>()
export function reconcileExtensions(entries: Record<string, EntryMeta>) {
  const signatures = Object.fromEntries(Object.entries(entries).map(([id, entry]) => [id, entry.files.join(',') + ':']))
  for (const [key, value] of extensions) if (!signatures[value.entryId] || !key.startsWith(signatures[value.entryId])) {
    for (const link of value.links) link.remove()
    extensions.delete(key)
  }
}
function extensionPage(entry: EntryMeta, route: string, entryId: string): Component<PageProps> | undefined {
  ...
  if (!extensions.has(key)) {
    ...
    for (const href of entry.files.filter(file => file.endsWith('.css'))) {
      if (!Array.from(document.querySelectorAll('link[rel=stylesheet]')).some(link => (link as HTMLLinkElement).href === new URL(href, location.href).href)) {
        const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = href; document.head.append(link); links.push(link)
      }
    }
...
export function PageOutlet(props: { path: string }) {
  ...
  const component = createMemo(() => {
    const page = selected()
    if (!page) return
    return builtins[page.module] ?? extensionPage(connection.state.entries[page.entryId], page.path, page.entryId)
  })
