...
export interface ClientBuildOptions { root: string; entry?: string; outDir?: string; sourcemap?: boolean }
...
export async function buildClient(options: ClientBuildOptions) {
  return build({
    configFile: false, root: options.root, base: './', plugins: [solid()],
    build: {
      target: 'es2022', outDir: options.outDir ?? 'dist', manifest: 'manifest.json', sourcemap: options.sourcemap ?? true,
