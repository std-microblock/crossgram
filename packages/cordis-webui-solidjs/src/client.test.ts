...
import { MAX_PENDING_RPC, MAX_REQUEST_BYTES, MAX_SOCKET_BUFFER } from './protocol.js'
...
class FakeSocket implements Socket {
  ...
  send = vi.fn((value: string) => { this.frames.push(JSON.parse(value)) })
  close = vi.fn((code?: number) => { this.readyState = 3; for (const fn of this.listeners.get('close') ?? []) fn({ code }) })
...
function setup(data: Record<string, any> = { count: 0, increment: () => 42 }) {
  const entry = { id: 'entry', data, toJSON: () => ({ module: 'debug', methods: ['increment'], files: [], routes: [] }), snapshot: () => ({ id: 'entry', data, cursor: {} }) }
  const webui = { version: 'solid-1', entries: { entry }, clients: {}, connectionChanged: vi.fn(), ctx: {} } as unknown as WebUI
  const socket = new FakeSocket()
  const client = new Client(webui, socket)
  webui.clients[client.id] = client
  const request = (type: string, body?: unknown) => client.receive({ data: JSON.stringify({ type, body }) })
...
describe('bounded subscription/RPC transport', () => {
  it('sends metadata without eagerly serializing large or sensitive entry state', () => {
    const { socket } = setup({ secret: 'never in metadata', history: 'x'.repeat(10_000_000) })
  ...
  it('subscribes with a complete snapshot, unsubscribes, and answers heartbeat', async () => {
    ...
    expect(socket.frames.at(-1)).toEqual({ type: 'entry:missing', body: { id: 'missing' } })
  })
  it('calls only exposed own methods and binds this to the entry', async () => {
    const { request, socket, entry } = setup({ increment(this: any, amount: number) { return this.id + ':' + amount }, explode() { throw Error('failure') } })
    await request('rpc:request', { sn: 1, entryId: entry.id, method: 'increment', args: [3] })
    expect(socket.frames.at(-1).body).toEqual({ sn: 1, ok: true, value: 'entry:3' })
    for (const method of ['constructor', 'toString', '__proto__', 'absent']) {
      await request('rpc:request', { sn: 2, entryId: 'entry', method, args: [] })
      expect(socket.frames.at(-1).body.ok).toBe(false)
    }
    await request('rpc:request', { sn: 3, entryId: 'entry', method: 'explode', args: [] })
    expect(socket.frames.at(-1).body).toEqual({ sn: 3, ok: false, message: 'failure' })
  })
  it('closes malformed, oversized and duplicate requests instead of throwing unhandled rejections', async () => {
    for (const data of ['{', 'null', JSON.stringify({ type: 'rpc:request', body: { sn: -1 } })]) {
    ...
    void pending.request('rpc:request', { sn: 1, entryId: 'entry', method: 'wait', args: [] })
    await pending.request('rpc:request', { sn: 1, entryId: 'entry', method: 'wait', args: [] })
    expect(pending.socket.close).toHaveBeenCalledWith(1007, expect.any(String))
  })
  it('bounds concurrent RPC calls', async () => {
    const { request, socket, client } = setup({ wait: () => new Promise(() => {}) })
    for (let sn = 0; sn < MAX_PENDING_RPC; sn++) void request('rpc:request', { sn, entryId: 'entry', method: 'wait', args: [] })
    await request('rpc:request', { sn: MAX_PENDING_RPC, entryId: 'entry', method: 'wait', args: [] })
