    ...
    try {
      ...
      await vi.waitFor(
        () =>
          expect(
            Object.values(
              (ctx.get('webui') as unknown as SolidWebUI)?.entries ?? {},
            ).some((entry) => entry.module === 'loader'),
          ).toBe(true),
        { timeout: 10_000 },
      )
      ...
      await page
        .getByText('Maximum is 65535', { exact: true })
        .first()
        .waitFor()
      expect(
        await page
      ...
      expect(
        await page.getByLabel('Secret', { exact: true }).getAttribute('type'),
      ).toBe('password')
      ...
      const entry = Object.values(
        (ctx.get('webui') as unknown as SolidWebUI).entries,
      ).find((entry) => entry.module === 'loader')!
      entry.mutate((data: any) => {
        data.entries = data.entries.map((item: any) => ({ ...item }))
      })
      ...
      await vi.waitFor(async () => {
        ...
        expect(saved).toContain('new.example.test')
        expect(saved).toContain('5443')
      ...
      await page
        .getByRole('dialog')
        .getByLabel('Installed plugin package')
        .fill(pathToFileURL(fixture).href)
      await page
        .getByRole('dialog')
        .getByLabel('Display name (optional)')
        .fill('Temporary plugin')
      await page
        .getByRole('dialog')
        .getByRole('button', { name: 'Create plugin' })
        .click()
      await page
        .getByRole('heading', { name: 'Temporary plugin', exact: true })
        .waitFor()
      await page
        .getByRole('button', { name: 'Remove plugin', exact: true })
        .click()
      await page
        ...
        .click()
      await page
        .getByRole('heading', { name: 'Everything in its right place' })
        .waitFor()
      await vi.waitFor(async () =>
      ...
      if (process.env.WEBUI_SCREENSHOTS) {
        ...
        await page.screenshot({
          path: join(process.env.WEBUI_SCREENSHOTS, 'solid-plugins-mobile.png'),
          fullPage: true,
          animations: 'disabled',
        })
