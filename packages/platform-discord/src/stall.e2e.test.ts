...
describe('Discord stall regression E2E', () => {
  it('projects a corrupt Discord cache through per-connection coalesced dialog RPCs', async () => {
    ...
    expect(ingestDialogs).toHaveBeenCalledTimes(connections.length)
    platform.stop()
  })
  ...
  it('keeps a five-thousand-channel guild out of the root dialog ingestion transaction', async () => {
    ...
    expect(getDialogs).toHaveBeenCalledTimes(connections.length)
