...
/** Short-lived, one-shot tickets used to copy a bridge login to another logical DC auth key. */
export class AuthTransferStore {
  ...
  has(id: Long, bytes: Uint8Array): boolean {
    const entry = this.getValid(id)
    return entry !== undefined && equalBytes(entry.bytes, bytes)
  }
