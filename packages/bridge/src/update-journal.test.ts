...
function delivery(eventKey: string, platformSessionId: string, pts: number) {
  return {
    eventKey, platformSessionId, pts, ptsCount: 1, seq: pts - 1, date: 100 + pts,
...
describe('MemoryUpdateDeliveryJournal', () => {
  ...
  it('tracks payload and publication while ordering pending rows by pts', async () => {
    ...
    expect((await journal.getAfter('session', 1, 1))[0]).toMatchObject({ eventKey: 'earlier', pts: 2 })
  })
  ...
  it('applies retention independently to each platform session', async () => {
    ...
    expect((await journal.getAfter('a', 1, 10)).map((row) => row.eventKey)).toEqual(['a-2', 'a-3'])
    expect((await journal.getAfter('b', 1, 10)).map((row) => row.eventKey)).toEqual(['b-1'])
  })
  ...
  it('supports zero retention without leaking event-key entries', async () => {
    ...
    expect(await journal.getAfter('session', 1, 10)).toEqual([])
  })
})
