...
const originalPng = new Uint8Array(Buffer.from(
...
async function createHarness(failSends = 0, sendImageDocumentsAsImages = true) {
  ...
  }, 1, undefined, undefined, undefined, undefined, { sendImageDocumentsAsImages })
...
describe('media send streaming', () => {
  it('sends image documents as byte-identical original images by default', async () => {
    const { rpc, uploads, inputs, consumed, peerId } = await createHarness()
    await uploads.savePart(session.platformSessionId, '40', 0, originalPng)

    const result = await rpc.sendMedia({
      _: 'messages.sendMedia', peer: peer(peerId), randomId: Long.fromNumber(40), message: '',
      media: {
        _: 'inputMediaUploadedDocument', file: inputFile(40, 1, 'original.png'), mimeType: 'image/png',
        attributes: [{ _: 'documentAttributeFilename', fileName: 'original.png' }],
      },
    }) as tl.RawUpdates

    expect(inputs[0].parts).toMatchObject([{
      type: 'media',
      media: {
        kind: 'image', name: 'original.png', mimeType: 'image/png',
        size: originalPng.length, width: 20, height: 10,
      },
    }])
    expect(consumed[0]).toHaveLength(1)
    expect([...consumed[0][0]]).toEqual([...originalPng])
    const update = result.updates.find((item) => item._ === 'updateNewMessage') as tl.RawUpdateNewMessage
    expect((update.message as tl.RawMessage).media).toMatchObject({
      _: 'messageMediaPhoto',
      photo: {
        _: 'photo',
        sizes: [expect.objectContaining({ _: 'photoSize', w: 20, h: 10, size: originalPng.length })],
      },
    })
  })

  it('keeps image documents as files when the bridge option is disabled', async () => {
    const { rpc, uploads, inputs, peerId } = await createHarness(0, false)
    await uploads.savePart(session.platformSessionId, '41', 0, originalPng)

    const result = await rpc.sendMedia({
      _: 'messages.sendMedia', peer: peer(peerId), randomId: Long.fromNumber(41), message: '',
      media: {
        _: 'inputMediaUploadedDocument', file: inputFile(41, 1, 'original.png'), mimeType: 'image/png',
        attributes: [{ _: 'documentAttributeFilename', fileName: 'original.png' }],
      },
    }) as tl.RawUpdates

    expect(inputs[0].parts).toMatchObject([{
      type: 'media', media: { kind: 'file', name: 'original.png', mimeType: 'image/png' },
    }])
    const update = result.updates.find((item) => item._ === 'updateNewMessage') as tl.RawUpdateNewMessage
