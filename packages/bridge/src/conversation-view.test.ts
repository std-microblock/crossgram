...
describe('ConversationViewService event facade', () => {
  it('dispatches account-scoped lookup and rendering through Cordis listeners', () => {
  ...
  it('falls through when no feature listener owns the conversation', () => {
    const service = createTestConversationViews()
    const ordinary = { id: 'ordinary', kind: 'group' as const, title: 'Ordinary' }
    expect(service.supports(ordinary)).toBe(false)
    expect(service.remember('session-a', 100, ordinary)).toBeUndefined()
  })
})
