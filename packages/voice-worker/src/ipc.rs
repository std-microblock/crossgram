...
impl<'a> PayloadReader<'a> {
    ...
    fn media_start_config(&mut self) -> Result<MediaStartConfig, IpcError> {
        ...
        for _ in 0..rtc_server_count {
            ...
            let host = String::from_utf8(self.bytes(MAX_HOST_BYTES)?)
                .map_err(|_| IpcError::Malformed)?;
            let username = String::from_utf8(self.bytes(MAX_HOST_BYTES)?)
                .map_err(|_| IpcError::Malformed)?;
            let password = String::from_utf8(self.bytes(MAX_HOST_BYTES)?)
                .map_err(|_| IpcError::Malformed)?;
